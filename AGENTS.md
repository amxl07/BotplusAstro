...
- Keep final runtime code, page copy, and docs minimal, readable, reusable, and maintainable.
- Every file, function, test, doc, script, section, and workflow must have a current job. When one is introduced or modified by the current change, remove it if the job is obsolete, duplicated, speculative, or only exists because an earlier implementation needed it.
- Prefer one source of truth. Do not keep duplicate docs, duplicate standards, generated/manual copies with conflicting ownership, or parallel automation paths.
- Do not add CI, sync automation, service connections, config layers, or scripts unless the operating workflow already requires them. Manual process is preferred when it is simpler and reliable.
- Build for multi-tenant enterprise use: preserve tenant isolation, security, privacy, auditability, and scalability.
- Do not hard-code customer, demo, tenant, or environment behavior into shared runtime paths; use configuration or persisted routing.
- Avoid logging secrets, access keys, credentials, raw PII, or sensitive customer data.
- Update docs only when behavior, usage, operations, API/config, or decision context changes; prefer editing the nearest existing doc over creating a new one.
- Keep documentation minimal: remove stale text, avoid duplicate explanations, and add only text that a future operator, developer, or buyer will use.

Mandatory minimization gate before finalizing:

- List every touched file and justify why it must remain changed; within each touched file, confirm every meaningful added or changed function, helper, component, test, config entry, branch, page section, workflow, and public contract is required for the request.
- For every new file, dependency, script, workflow, config shape, or exported/public surface, confirm existing code/content cannot do the job with a smaller change.
- For existing code or content touched by the current change, check whether it can be simplified, reused, or made more efficient within the current scope.
- Confirm the diff avoids the named failure modes: Kitchen Sink, Wrong Abstraction, Optimistic Path, and Runaway Refactor.
- Remove anything introduced or modified by the current change that fails that review before handoff.

Required review passes for substantial changes:

1. Plan: define scope, assumptions, tradeoffs, risks, tenant/security impact, success criteria, and focused test cases.
2. Tests: create or update tests before implementation where practical; for bug fixes, reproduce the failure first.
3. Implementation: make the smallest change that satisfies the tests and goal.
4. Minimization: run the mandatory minimization gate; remove unnecessary files, functions, helpers, components, config, tests, docs, and workflows introduced or modified by the current change before handoff.
5. Verification: run focused tests/builds/screenshots or review checks and critique the result.
6. Documentation: update only the minimal docs needed for the change.
